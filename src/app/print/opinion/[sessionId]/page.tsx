...
export default async function PrintOpinionPage({
  params,
  searchParams,
}: {
  ...
  if (submitted.length === 0) return notFound();

  return (
    <html lang="ko">
      <head>
        <meta charSet="UTF-8" />
        <title>{session.companyName} 투심의견서 수합본</title>
        <style>{`
          * { margin: 0; padding: 0; box-sizing: border-box; }
          ...
          @media print { .print-bar { display: none; } .print-wrap { margin-top: 0; } }
        `}</style>
      </head>
      <body>
        <div className="print-bar no-print">
          <span>📄 {session.companyName} 투심의견서 수합본 — {submitted.length}명</span>
        ...
        <div className="print-wrap">
          {submitted.map((reviewer) => {
            ...
            return (
              <div key={reviewer.id} className="page">
                <div className="header">
                  <div className="company">{session.companyName}</div>
